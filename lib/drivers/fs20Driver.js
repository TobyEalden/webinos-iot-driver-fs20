  ...
  function sendDeviceData(device) {
    for (var svc in device.config.services) {
      ...
      if (typeof data !== "undefined") {
        callbackFunc('data', device.config.services[svc].serviceId, data.toFixed(2) );
      }
    }
  }
        ...
        if (existsSync(CONFIG_PATH)) {
          ...
          for (var dev in configData.devices) {
            ...
            for (var svc in device.config.services) {
              ...
              var desc = svcData.name !== svcData.subType ? svcData.name + " " + svcData.subType + " [" + dev + "]" : svcData.name + " [" + dev + "]";
              var service_info = {
                type: svcData.subType,
                name: configData.devices[dev].name + " - " + svcData.name + " (" + svcData.units + ")",
                description: desc
              };
